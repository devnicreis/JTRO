...
<div class="presencas-layout">
    <div class="presencas-coluna">
        <div class="presencas-card">
            ...
            <div class="reuniao-visualizar-acoes">
                <a class="btn-presenca-oracao" href="/pedidos_oracao.php?reuniao_id=<?php echo (int) $reuniao['id']; ?>">
                    <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"></svg>
                    🛐 Pedidos de Oração
                </a>
                <a class="botao-link" href="/presencas.php?grupo_id=<?php echo (int) $reuniao['grupo_familiar_id']; ?>&data=<?php echo urlencode($reuniao['data']); ?>">
                    🖊️ Editar Reunião
                </a>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($listaPresencas as $presenca): ?>
                            <?php
                            $status = strtolower((string) ($presenca['status'] ?? ''));
                            $statusPrincipal = ucfirst((string) ($presenca['status'] ?? ''));
                            $presenteTempo = (string) ($presenca['presente_tempo'] ?? '');
                            $ausenciaTipo = (string) ($presenca['ausencia_tipo'] ?? '');
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="reuniao-status-celula">
                                        <span class="reuniao-status-principal <?php echo $status === 'presente' ? 'presente' : ($status === 'ausente' ? 'ausente' : ''); ?>">
                                            <?php echo htmlspecialchars($statusPrincipal); ?>
                                        </span>

                                        <?php if ($status === 'presente' && $presenteTempo !== ''): ?>
                                            <div class="reuniao-status-meta">
                                                <?php if ($presenteTempo === 'no_horario'): ?>
                                                    <span class="badge badge-green">No horário</span>
                                                <?php elseif ($presenteTempo === 'atrasado'): ?>
                                                    <span class="badge badge-amber">Atrasado</span>
                                                <?php endif; ?>
                                            </div>
                                            <?php if ($presenteTempo === 'atrasado' && $justificativaAtraso !== ''): ?>
                                                <div class="reuniao-status-justificativa">
                                                    <?php echo htmlspecialchars($justificativaAtraso); ?>
                                                </div>
                                            <?php endif; ?>
                                        <?php endif; ?>

                                        <?php if ($status === 'ausente' && $ausenciaTipo !== ''): ?>
                                            <div class="reuniao-status-meta">
                                                <?php if ($ausenciaTipo === 'justificada'): ?>
                                                    <span class="badge badge-blue">Justificada</span>
                                                <?php elseif ($ausenciaTipo === 'injustificada'): ?>
                                                    <span class="badge badge-red">Injustificada</span>
                                                <?php endif; ?>
                                            </div>
                                            <?php if ($ausenciaTipo === 'justificada' && $justificativaAusencia !== ''): ?>
                                                <div class="reuniao-status-justificativa">
                                                    <?php echo htmlspecialchars($justificativaAusencia); ?>
                                                </div>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    </div>
